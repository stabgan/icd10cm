...
async function buildElectronApp() {
  try {
    ...
    exec('npx electron-builder build --win --x64 --dir --config.win.target=portable --config.forceCodeSigning=false --config.npmRebuild=false --config.electronDist=node_modules/electron/dist');
